...
import { ProjectsProjectIdBoard, ProjectsProjectIdTickets } from '@/routes'
...
export default function ProjectOverviewPage() {
    ...
    const priorityDistribution = priorities.map((priority) => {
        const count = tickets.filter((ticket) => {
            return (
                ticket.priority?.id === priority.id ||
                ticket.priority.id === priority.id
            )
        }).length
        return {
    ...
    const typeDistribution = types.map((type) => {
        const count = tickets.filter((ticket) => {
            return ticket.type?.id === type.id || ticket.type.id === type.id
        }).length
        return {
    ...
    if (
        ...
        return (
            <div className="p-8">
                <div className="bg-muted h-20 w-full animate-pulse rounded-md"></div>
                <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-3">
                    <div className="bg-muted h-40 animate-pulse rounded-md"></div>
                    <div className="bg-muted h-40 animate-pulse rounded-md"></div>
                    <div className="bg-muted h-40 animate-pulse rounded-md"></div>
                </div>
            </div>
                ...
                <Card>
                    ...
                    <CardContent>
                        ...
                        <p className="text-muted-foreground text-xs">
                            {tickets?.filter((t) => t.status?.name === 'Done')
                                .length || 0}{' '}
                            completed
                        </p>
                    </CardContent>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="flex space-x-4">
                            ...
                            <Button variant="outline" size="sm" asChild>
                                <ProjectsProjectIdTickets.Link projectId={project?.id}>
                                    <FileSpreadsheet className="mr-2 h-4 w-4" />
                                    List
                                </ProjectsProjectIdTickets.Link>
                            </Button>
                        </div>
                    </CardContent>
                </Card>
                        ...
                        {recentTickets.length > 0 ? (
                            <div className="space-y-4">
                                {recentTickets.map((ticket) => {
                                    const status = ticket.status?.color
                                        ? ticket.status
                                        : statuses.find(
